...
void callback(const PointCloud2::ConstPtr& camera_cloud, const pcl_msgs::ModelCoefficients::ConstPtr& cam_plane_coeffs){
    ...
    do{
        ...
        //    if(DEBUG) ROS_INFO ("LINE: %lu inliers", inliers->indices.size ());
        //}
        ...
        if (fabs(coefficients->values[3])>=min_border_x_){
            ...
            else if (fabs(coefficients->values[0]-first_line_x)<min_distance_between_borders_x_){ /*if(DEBUG) ROS_INFO("Invalid line (proximity)"); */continue;}
        }else if(fabs(coefficients->values[4])>=min_border_x_){
            if (first_line_y<0.0) first_line_y = coefficients->values[1];
            else if (fabs(coefficients->values[1]-first_line_y)<min_distance_between_borders_y_){ /*if(DEBUG) ROS_INFO("Invalid line (proximity)"); */continue;}
        }

        if(DEBUG) ROS_INFO("[Camera] Removed line: %d inliers / orientation: %f %f %f", last_inliers, coefficients->values[3], coefficients->values[4], coefficients->values[5]);

        /*if (it==0) first_line_x = coefficients->values[0];
        else if (fabs(coefficients->values[0]-first_line_x)<min_distance_between_borders_){
            //it--;
            continue;
        }*/
    ...
    do{
        ...
        // PointCloud2 circles_ros2;
        // pcl::toROSMsg(*circle_cloud, circles_ros2);
        // circles_ros2.header = camera_cloud->header;
        // circles_pub2.publish(circles_ros2);
    ...
    if(DEBUG) ROS_WARN( "[Camera] %d/%d frames: %ld pts in cloud", images_used_, images_proc_, cumulative_cloud->points.size());
...
int main(int argc, char **argv){
    ...
    //nh_.param("cluster_frames", cluster_frames_, 30);
