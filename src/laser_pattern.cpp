...
ros::Publisher cumulative_pub, centers_pub, pattern_pub, range_pub, transf_pub, coeff_pub, aux_pub, auxpoint_pub;
...
void callback(const PointCloud2::ConstPtr& laser_cloud){
    ...
    pcl::toROSMsg(*velo_filtered, range_ros); // velocloud
    ...
    plane_segmentation.setModelType (pcl::SACMODEL_PARALLEL_PLANE);//pcl::SACMODEL_PERPENDICULAR_PLANE);
    ...
    float THRESHOLD = 0.5 ; //0.1 if normalized
    ...
    //pcl::SampleConsensusModelPlane<Velodyne::Point>::Ptr dit (new pcl::SampleConsensusModelPlane<Velodyne::Point> (edges_range_cloud));
    ...
    //pcl::copyPointCloud<Velodyne::Point>(*edges_range_cloud, inliers2, *pattern_cloud);
    ...
    pcl::toROSMsg(*circles_cloud  , velocloud_ros2); // circles_cloud
    ...
    // TODO Get circle edges centroid
    ...
    circle_segmentation.setDistanceThreshold (0.04); // 0.04
    ...
    pcl::copyPointCloud<pcl::PointXYZ>(*xy_cloud, *copy_cloud); // TODO CHANGED circles_cloud
    ...
    while ((copy_cloud->points.size()+centroid_cloud_inliers.size()) > 3 && found_centers.size()<4 && copy_cloud->points.size()){
        ...
        {
            // No circle found, try adding wrong removed points before exiting
            //   if(DEBUG) ROS_INFO("Adding back %lu points", centroid_cloud_inliers.size());
            //   for (std::vector<pcl::PointXYZ>::iterator pt = centroid_cloud_inliers.begin(); pt < centroid_cloud_inliers.end(); pt++){
            //       copy_cloud->push_back(*pt);
            //   }
            //   circle_segmentation.setInputCloud (copy_cloud);
            //   circle_segmentation.segment (*inliers3, *coefficients3);
            //   if (inliers3->indices.size () == 0)
            ...
            //       break;
            //   }
            break; // TODO REMOVE FOR 16 BEAMS
        }
        ...
        center.z = zcoord_xyplane;//(*circle_cloud->points.begin()).z;
        ...
        if (centroid_distance<0.15){
            valid = false;
            //    centroid_cloud = circle_cloud->makeShared();
            ...
            // Convert PointCloud to ROS format and publish
            sensor_msgs::PointCloud2 ros_pointcloud;
            pcl::toROSMsg(*centroid_cloud, ros_pointcloud);
            ros_pointcloud.header = laser_cloud->header;
            aux_pub.publish(ros_pointcloud);
        }else if(centroid_distance > 0.25){
            valid = false;
        }else{
            ...
            //    if(DEBUG) ROS_INFO("Check if %lu schrodinger points belong to it", centroid_cloud->size());
        ...
        if (valid){
            ...
            //    if(DEBUG) ROS_INFO("%f %f %f", center.x, center.y, center.z);
            //
        ...
        // Convert PointCloud to ROS format and publish
        // sensor_msgs::PointCloud2 ros_pointcloud;
        //  pcl::toROSMsg(*circle_cloud, ros_pointcloud);
        // ros_pointcloud.header = laser_cloud->header;
        // aux_pub.publish(ros_pointcloud);
        // if(DEBUG) ROS_INFO("PC published");
        //
        // usleep(2000000);
    }
    ...
    if(found_centers.size() >=min_centers_found_ && found_centers.size() < 5){
        ...
        sensor_msgs::PointCloud2 ros_pointcloud;
        pcl::toROSMsg(*cumulative_cloud, ros_pointcloud); //circles_cloud
        ros_pointcloud.header = laser_cloud->header;
    ...
    //      pcl::toROSMsg(*centers_cloud, ros2_pointcloud);
    //      ros2_pointcloud.header = laser_cloud->header;
    //      centers_pub.publish(ros2_pointcloud);
    //      if(DEBUG) ROS_INFO("Pattern centers published");
    //  }
}
...
int main(int argc, char **argv){
    ...
    centers_pub = nh_.advertise<velo2cam_calibration::ClusterCentroids> ("centers_cloud", 1); // TODO REMOVE ONLY ONE NEEDED
    pattern_pub = nh_.advertise<PointCloud2> ("xy_pattern", 1);
    range_pub = nh_.advertise<PointCloud2> ("filtered_velo", 1);
    transf_pub = nh_.advertise<PointCloud2> ("transf_velo", 1);
    coeff_pub = nh_.advertise<pcl_msgs::ModelCoefficients> ("plane_model", 1);
    aux_pub = nh_.advertise<PointCloud2> ("wrong_circle", 1);
